          ...
          <div className="space-y-6 pb-20">
            {isLoading ? (
              ...
              filteredPosts.map((post) => (
                <article key={post.id} className="bg-surface-container-lowest border-2 border-primary rounded-2xl p-5 md:p-6 bubbly-card">
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-on-surface-variant opacity-70 uppercase tracking-wider">{post.phase}</p>
                    </div>
                  </div>

                  <p className="font-headline-md text-base md:text-[20px] leading-relaxed text-on-surface mb-5 whitespace-pre-wrap">
                    "{post.content}"
                  </p>

                  {/* Action Buttons styled like CycleCircle mockup */}
                  <div className="flex flex-wrap gap-2 md:gap-3 pt-4 border-t border-outline-variant">
                    <button
                      onClick={() => handleReaction(post.id, "red_flag_count")}
                      disabled={!!reactedPosts[post.id] || !session}
                      className={`flex items-center gap-1.5 md:gap-2 px-3 py-1.5 md:px-4 md:py-2 border-2 rounded-full transition-transform active:scale-95 ${reactedPosts[post.id] === "red_flag_count"
                    ...
                    >
                      ...
                      <span className="bg-primary text-on-primary text-[9px] md:text-[10px] px-1.5 py-0.5 rounded-full ml-0.5 md:ml-1 font-bold">{post.red_flag_count}</span>
                    </button>

                    <button
                      onClick={() => handleReaction(post.id, "trust_gut_count")}
                      disabled={!!reactedPosts[post.id] || !session}
                      className={`flex items-center gap-1.5 md:gap-2 px-3 py-1.5 md:px-4 md:py-2 border-2 rounded-full transition-transform active:scale-95 ${reactedPosts[post.id] === "trust_gut_count"
                    ...
                    >
                      ...
                      <span className="bg-primary text-on-primary text-[9px] md:text-[10px] px-1.5 py-0.5 rounded-full ml-0.5 md:ml-1 font-bold">{post.trust_gut_count}</span>
                    </button>

                    <button
                      onClick={() => handleReaction(post.id, "give_time_count")}
                      disabled={!!reactedPosts[post.id] || !session}
                      className={`flex items-center gap-1.5 md:gap-2 px-3 py-1.5 md:px-4 md:py-2 border-2 rounded-full transition-transform active:scale-95 ${reactedPosts[post.id] === "give_time_count"
                    ...
                    >
                      ...
                      <span className="bg-primary text-on-primary text-[9px] md:text-[10px] px-1.5 py-0.5 rounded-full ml-0.5 md:ml-1 font-bold">{post.give_time_count}</span>
                    </button>
                  </div>
